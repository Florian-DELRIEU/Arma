...
if (!(_current_unit in switchableUnits) && !(side _current_unit == civilian)) then {
	...
	if (true) then {
		...
		_current_unit addItemToUniform "ACE_EarPlugs";
	...
	if (_is_medic) then {
		...
		for "_i" from 1 to 20 do {_current_unit addItemToBackpack "ACE_packingBandage";};
		for "_i" from 1 to 5 do {_current_unit addItemToBackpack "ACE_tourniquet";};
		for "_i" from 1 to 15 do {_current_unit addItemToBackpack "ACE_morphine";};
		...
		for "_i" from 1 to 2 do {_current_unit addItemToBackpack "ACE_EarPlugs";};
	};
};